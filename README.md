...
1.	Break up work into smaller components using Stata .do files.  
        Used 5 do files (and 1 main)  
1.	Read .csv data in in Stata.  
        Yes  
1.	Fix common data quality errors in Stata (for example, string vs number, missing value).  
        Changed some variables from string to bools (created dummy variables)  
1.	Aggregate, reshape, and combine data for analysis in Python or Stata. Demonstrate at least one of these data manipulations.  
        Aggregated data by countries to created graphs  
1.	Prepare a sample for analysis by filtering observations and variables and creating transformations of variables. Demonstrate all three.  
...
1.	Save data in Stata.  
        Saved data in Data\Derived\prices_all_clean.dta  
1.	Run ordinary least squares regression in Stata.  
        Ran in Code\5_analysis.do  
1.	Create a graph (of any type) in State.  
        Created several in Graphs\  
1.	Save regression tables and graphs as files. Demonstrate both.  
        Saved in Graphs\ and Tables\  
1.	Install a Stata package. (Can be the same as we already did in class.)  
        Installed in Code\1_install_packages.do  
1.	Commit files into git version control.  
        Commited  
1.	Name files, functions and variables appropriately.  
        Okay  
1.	Write helpful comments and commit messages. Demonstrate both.  
        Wrote  
1.	Push a repository to GitHub.  
        Push  
1.	Explain how to reproduce your work with short instructions or a self-documenting master script.  
        Explained above  
